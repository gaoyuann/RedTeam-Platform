...
export default function up(db) {
  // Import the 3 generic web_url playbooks that were added after initial migration
  const newPlaybooks = [
    'web_auto_killchain.json',
    'web_file_upload_exploit.json',
    'web_vuln_exploit.json',
  ];
  ...
  const tx = db.transaction(() => {
    for (const file of newPlaybooks) {
      const filePath = join(PROJECT_PLAYBOOKS_DIR, file);
      let pb;
      try {
        pb = JSON.parse(readFileSync(filePath, 'utf-8'));
      } catch (err) {
        console.log(`[Migration 014] File not found: ${file}, skipping`);
        continue;
      }

      const row = extractPlaybookRow(pb, false);
  ...
  tx();
  console.log(`[Migration 014] Imported ${count} generic web playbooks`);
}
